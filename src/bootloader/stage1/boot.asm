org 0x7C00                    ; Assembler directive. Tells NASM to assume the
                              ...
                              ; This is not an instruction to the loader.
                              ; BIOS will also load the your boot sector 
                              ; from floppy to the same address. As we are
                              ; in primitive stage of Os loading.
bits 16                       ; directive to tell that generate 16 bit 
                              ; instructions. In 16-bit address mode, CPU
                              ; uses: physical address = segment * 16 + offset
                              ; where segment = DS, CS, ...
                              ; The reason I'm telling this there are multiple
                              ; ways to reach to the same physical address
                              ; set DS = 0, and offset = 0x7c00
                              ; or set DS = 0x7c00. The combination needs to 
                              ; work

; symbol = new line. More details (https://www.rfc-editor.org/info/rfc20/)
%define ENDL 0x0D, 0x0A       

;
; FAT12 header 
; -------------
; FAT 12 was designed for the floppy disk and manage a maximum size of 16MB
; because it uses 12 bit to address the cluster
; 
; Boot Record
; ------------
; The boot record occupies one sector, and is always placed in logical 
; sector number zero of the "partition"
;
...
;
jmp short start            ;  Since the first sector of the disk is loaded 
                           ; into ram at location 0x0000:0x7c00 and executed
                           ; without this  the processor would attempt to 
                           ; execute data that isn't code
nop                        ; 3 bytes jmpm short <entry-point> 
bdb_oem_identifier:        db 'MSWIN4.1'   ; 8 byte -  If the string is less 
                                           ; than 8 bytes, it is padded with 
                                           ; spaces.
...
start:
    ...
    ; We can not set directy ds, es, ss to 0, thus doing it via ax,
    ...
    mov sp, 0x7C00     ; stack pointer to the beginning of our program 
                       ; (or below our bootloader address)
...
.after:
    ...
    ; BIOS discover the value for dl - to remember which physical/logical
    ; BIOS drive it was booted from, so that every later INT 13h disk read
    ; uses the same drive. The value it self is not the FAT12 information
    ; it is information passed from BIOS to your bootloader
    ; The momenty bootloader is loaded and it start it intrudction, it good
    ; that it keep what it need to get from the BIOS, so that later it can use
    ; it. Becasue BIOS role finished after loading bootloader
    mov [ebr_drive_number], dl  ; This is in-memory 
    ...
    int 13h            ; disk service routine (return dl also as one 
                       ; thus preserved earlier) but this is not the only reason
                       ; to preserve. Regardless we have to save. But must
                       ; as early
    jc floppy_error    ; jump if carry - CF is set if error, else CF = 0
    pop es
    ...
    ;
    and cl, 0x3F       ; remove top 2 bits - cl contains the maximum sector 
                       ; number i.e., last
    xor ch, ch         ; safer way to set 0 in assembely. You need not to worry 
                       ; about how many bits are used by ch
    ...
    ; This is kind of fix thus we can refere to BDB ? I believe..  
    ; may be specification set already ?
    mov ax, [bdb_sectors_per_fat]  ; the bdb_sectors_per_fat was dw i.e., 
                                   ; 2 bytes thus it will take ah, al 
                                   ; (8 bits each)
    mov bl, [bdb_fat_count]        ; Notice BL was populated by BL = drive 
                                   ; type (AT/PS2 floppies only)
                                   ; but didn't saved bl (push bl) because 
                                   ; we dont' need that info..
                                   ; note that dbb_fat_count was db (1 byte) 
                                   ; and bl is also 8 bits 
    
    ; formula lba = bootsector.reservedSectors + sector_per_fat * count_fat
    xor bh, bh

    ; after mul the ax will have = sector_per_fat * count_fat
    mul bx                         ; mul instruction has 2 operand; one is 
                                   ; specified and other is implicit i.e., ax
                                   ; ax = ax * bx
    ...
    xor dx, dx                        ; dx = 0
    div word [bdb_bytes_per_sector]   ; DX = remainder, AX = Quotient 
                                      ; (number of sector)

    ; is a way to check if dx == 0.. if dx = 0, then only dx, dx i.e, 
    ; bitwise of dx & dx = 0
    test dx, dx 
    jz .root_dir_after
...
.root_dir_after:
    ...
    mov cl, al                      ; div contains the number of sector in AX, 
                                    ; so move that value to the CL
                                    ; cl = number of sector to read = size of 
                                    ; root directory
    pop ax                          ; get back the value of LBA of root 
                                    ; directory
    mov dl, [ebr_drive_number]      ; driver number
    mov bx, buffer                  ; es:bx = buffer (es:bx: memory address 
                                    ; where to store read data)
                                    ; in the ES section at the offset bx 
                                    ; ES:BX -> data buffer (see the link)
    ...
    xor bx, bx 
    mov di, buffer                  ; di point the address where the we have 
                                    ; read root directory section
    ; as file name is the first field in the structure thus the di will 
...
.search_kernel:
    ...
    mov cx, 11                      ; length of file_stage2_bin name i.e. 
                                    ; 11 char
    push di                         ; save the value of di, which points to the
                                    ; root director start 
    ; note that di point to the begin of entry which is essentially the file 
    ; name which is fixed 11 byte
    ; repeat while equal or until cx reach to the 0, cx is decremented on 
    ; each iteration

    ; this will incremente di = di + 1, and si = si + 1 until we iterate to the
    ;  length of si, i.e, cx 
    ; each iteration cx = cx - 1
    ; for (int i = cx - 1; i >= 0 ; --i) { if (si == di), then si++, di++}
    repe cmpsb                      ; compare string byte (compare two byte, 
                                    ; si and di) ds:si, es:di
                                    ; si and di are incremented when direction 
                                    ; flag = 0, else =1 then decremented
    pop di 
    ; if both string are same
    je .found_kernel                ; move to the kernel label
    ; point di to the next entry in the root directory and repeat again, until 
    ; when ? until bx reach the number of entry cont
    add di, 32                      ; di = di + 32 (size of each entry in the 
                                    ; root directory)
...
.found_kernel:
    ...
    ; 26 offset in an 32 byte entry in the root file. so check the 
    ; current i.e., di starting entry and read the value 
    mov ax, [di + 26]               ; to get the first cluster, you read 2 byte 
                                    ; i.e, 26, 27 into the ax 
                                    ; which is the staring cluster number, 
                                    ; now go for the chain
    mov [stage2_cluster], ax        ; store the value, mov 2 byte (ax) to a 
                                    ; word (2byte) location stage2_cluster
    ...
    mov bx, buffer                  ; es:bx, es = 0, bx = buffer, means 
                                    ; disk_read will write to es:bx
    ...
    ; since we are in 16-bit, we can't access memory = 2^16 = 640KiB 
    ; RAM ("lower memory")
    ; We have a contigous are between bootloader 07C00 + 512 Byte = 
    ; 0x7DFF and extended BIOS Data area
    ...
    ;       30KiB Conventional memory - 30KiB. If you notice we use this 
    ; for the stack 
...
.load_kernel_loop:
    ...
    add ax, 31                      ; first_cluster = (cluster_number - 2) 
                                    ; * sectors_per_cluster + start_sector
                                    ; start_sector = reserved + fat + 
                                    ; root directory size = 1 + 18 + 13 = 32
    ...
    mov ax, [stage2_cluster]        ; fetch the saved value of first cluster 
                                    ; (index of first cluster in the fat)
    mov cx, 3                       ; 3/2 as 1.5 byte each entry i.e., 12 bit 
                                    ; in FAT 12 table
    mul cx                          ; ax = ax * cx (cx = 3)
    mov cx, 2                       ; cx = 2
    div cx                          ; ax = ax / cx, and ax = quotient, 
                                    ; dx = remainder
    ...
    mov si, buffer                  ; mark si to the begin the buffer region, 
                                    ; where disk_read has written
    add si, ax                      ; si = si + source index (offset) 
                                    ; = FAT_OFFSET
    mov ax, [ds:si]                 ; read entry from FAT table at index ax 
                                    ; si = buffer, and ds = 0; 
                                    ; Physical address = ds * 16 + si 
                                    ; thus mov ax, [0:]
    or dx, dx                       ; Test if remainder (dx) is 0
    jz .even                        ; If 0, jump to even logic
;
; e.g.,
...
.read_finish:
    ...
    cli                             ; disable interrupt that way cpu cant' 
                                    ; get out of "halt" state
    hlt 
;
...
puts:
    ...
    push si            ; SI is set to the start address, we will read 
                       ; starting this
    push ax            ; save initial value of ax which is 0
    push bx

.loop:
    lodsb              ; load next character in al and increment si by 
                       ; # of byte loaded (load string byte)
    and al, al         ; verfiy if next character is null
    jz .done           ; previous instruction return 0 then jump

    mov ah, 0x0E       ; BIOS teletype output
    mov bh, 0          ; display page
    int 0x10           ; calls BIOS; 10H
...
.halt:
    cli             ; disable interrupt, this way we can't get out of 
                    ; "halt" state
    hlt
;
; Disk routines
...
lba_to_chs:
    ...
    xor dx, dx             ; we reset dx
                           ; reading few next line will justify the 
                           ; reason of this dx is first used during division 
                           ; i.e., dx:ax / <2-byte-number>
                           ; and finally used to store the quotient

    ; word tells the CPU to read 2 bytes = 16-bit from the memory address 
    ; of bdb_sectors_per_track
    ...
    xor dx, dx
    div word [bdb_heads]                 ; ax = 
                                         ; (LBA / sectorpertrack) / heads 
                                         ;                    = cylinder
                                         ; dx = 
                                         ; (LBA / sectorpertrack) % heads = head 

    mov dh, dl                           ; dh = head 
    mov ch, al                           ; ch = cylinder (lower 8 bits)
    shl ah, 6                            ; shift left (bring bits from last 2 
                                         ; to front 2 so that we can do bitwise 
                                         ; OR in the  next step)
    or cl, ah

    pop ax                               ; temporary pop the dx to ax, and then 
                                         ; we will only take al to dl 
    mov dl, al
    pop ax
    ret 
...
disk_read: 
    ...
    push cx                     ; save CL, because call to the lba_to_chs 
                                ; override cx
    call lba_to_chs             ; LBA -> CHS, see the return contract
    pop ax                      ; AL = number of sector to read (the top of 
                                ; the stack which was cx) pop the top of stack 
                                ; to the ax i.e., original cl value
    
    mov ah, 0x02                ; BIOS read_sector function code

    ; reading file systems off magentic media requires a fault tolerant 
    ; read loop because mechanical hardware is prone to intermittent 
    ; read failures thus we will retry three time, set the di to 3
    mov di, 3                   ; init retry count to 3 

.retry:
    pusha                       ; save all the register (layer 2), we don't 
                                ; know what bios modifies
    stc                         ; Failsafe: set the carry flag manually, 
                                ; some BIOS doesn't set it
    int 0x13                    ; Call bios disk service, if carry flag 
                                ; cleared (0) = success
    jnc .done                   ; jump if carry is not 
                                ; set(1) - (which mean it is 0) 
...
KERNEL_LOAD_SEGMENT     equ 0x2000     ; equ means no memory allocate for the 
                                       ; constant, kind of #define
